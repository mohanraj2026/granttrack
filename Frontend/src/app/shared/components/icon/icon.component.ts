 ...
 * Usage: <gt-icon name="dashboard" [size]="18" />
 * Self-contained — no external icon font/dependency, fully themeable via `color`.
 */
export type IconName =
  | 'dashboard' | 'file-text' | 'scale' | 'wallet' | 'award' | 'landmark'
  | 'trending-up' | 'book' | 'bell' | 'users' | 'user' | 'menu' | 'chevron-down'
  | 'chevron-left' | 'chevron-right' | 'search' | 'check-circle' | 'x-circle'
  | 'info' | 'alert-triangle' | 'inbox' | 'clipboard' | 'plus' | 'edit' | 'trash'
  | 'eye' | 'check' | 'x' | 'log-out' | 'briefcase' | 'layers' | 'shield-check'
  | 'building' | 'diamond' | 'calendar' | 'lock' | 'mail' | 'send' | 'flask'
  | 'file-check' | 'dollar' | 'settings' | 'external-link' | 'sparkles' | 'more-horizontal'
  | 'pause-circle' | 'check-square' | 'play-circle' | 'trash-2' | 'upload-cloud' | 'download' | 'sun' | 'moon';
...
const ICONS: Record<IconName, string> = {
  ...
  send: '<path d="M14.54 5.46a2 2 0 0 0-2.83-2.83l-9.42 9.42a2 2 0 0 0 0 2.83l5 5a2 2 0 0 0 2.83 0Z"/><path d="m22 2-11 11"/>',
  flask:
  ...
  sparkles:
    '<path d="M9.94 14.06 8 21l-1.94-6.94L-.88 12.12 6.06 10.18 8 3.24l1.94 6.94 6.94 1.94Z" transform="translate(4 0)"/><path d="M18 5h.01"/><path d="M20 12h.01"/>',
  'more-horizontal':
...
@Component({
  ...
  template: `<span class="gt-icon" [style.width.px]="size" [style.height.px]="size" [innerHTML]="svg()"></span>`,
  styles: [
    `
      .gt-icon { display: inline-flex; align-items: center; justify-content: center; line-height: 0; flex-shrink: 0; }
...
export class IconComponent {
  ...
  readonly svg = computed<SafeHtml>(() => {
    const body = ICONS[this._name()] ?? ICONS['info'];
    const markup =
      `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="${this.size}" height="${this.size}" ` +
      `fill="none" stroke="currentColor" stroke-width="${this.strokeWidth}" stroke-linecap="round" stroke-linejoin="round">${body}</svg>`;
